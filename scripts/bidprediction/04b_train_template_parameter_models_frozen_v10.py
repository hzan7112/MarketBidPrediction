...
PRICE_CANDIDATES = [
    ...
    "RandomForest",
    "ExtraTrees",
    "RidgePlusRandomForest",
    "RidgePlusExtraTrees",
    "RidgePlusExtraTrees_weighted",
]
...
def fit_price_predictor(
    ...
    """
    Ridge handles global/temporal extrapolation.
    Tree ensembles model only nonlinear residual structure in hybrid modes.
    """
    Y = state["Z_train"]
    ...
    if candidate in {
        ...
        tree_name = (
            "RandomForest"
            if candidate == "RidgePlusRandomForest"
            else "ExtraTrees"
        )
...
def predict_price_predictor(
    ...
    if kind in {
        ...
        return np.asarray(
            predictor["tree"].predict(X),
            dtype=float,
        )

    if kind.startswith("RidgePlus"):
        linear = np.asarray(
            predictor["ridge"].predict(Xs),
            dtype=float,
        )
...
def main():
    ...
    print("=" * 80)
    print(
        "Template parameter regression - direct 21-point price curve + linear/tree hybrid"
    )
    print("=" * 80)
    print(
    ...
    bundle = {
        "version": "04b-direct-price-hybrid-v10",
        "model_mode": "direct_21_price_hybrid_q_specialist",
    ...
    cfg = {
        ...
        "hybrid_price_model": (
            "Ridge global trend + tree ensemble residual"
        ),
    ...
    lines = [
        ...
        (
            "  hybrid candidates = Ridge global trend "
            "+ tree nonlinear residual"
        ),
        "",
        "Validation price:",
        (
