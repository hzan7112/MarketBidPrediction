...
def main():
    ...
    if not test_curve_parts:
        raise ValueError(
            "Frozen test_curve parts are empty."
        )
    ...
    if (
        ...
        != "pca_price_q_specialist"
    ):
        raise RuntimeError(
            "This 04c expects the v8 04b PCA-price bundle. "
            "Run the updated 04b first."
        )
    ...
    print("=" * 80)
    print(
        "Final bid-curve prediction - PCA residual price representation"
    )
    print("=" * 80)
    print(
        f"Year:                  {args.year}"
    ...
    print(
        ...
        "p_anchor + p_span * (template + PCA residual)"
    )
    print(
        "Price PCA components:  "
        f"{parameter_bundle['price_basis']['n_components']}"
    )
    print(
        "Price PCA variance:    "
        f"{parameter_bundle['price_basis']['explained_variance_cumulative']:.4%}"
    )
    print(
        "Price anchor target:   "
        f"{parameter_bundle['price_target_state']['anchor_mode']}"
    )
    print(
        "Price model:           "
    ...
    for i, p in enumerate(
        ...
        # Quantity truth for metrics. Price columns are deliberately zero:
        # v8 price metrics are evaluated directly from reconstructed curves.
        true_theta = v8_theta_for_metrics(
            num(
                d[
                    "theta_q_base_mw"
                ]
        ...
        # 1) New representation oracle:
        # true template + true physical price scale +
        # PCA projection of the true residual shape.
        oracle = v8_oracle_representation(
            parameter_bundle,
            d,
            true_t_s,
            centers,
        ...
        # 2) Parameter model only:
        # true template + predicted price/quantity parameters.
        pred_oracle_t = v8_predict_parameters(
            parameter_bundle,
            d,
            true_t_s,
            centers,
        ...
        # 3) Full pipeline:
        # predicted template + predicted price/quantity parameters.
        origin_valid = (
            norm(
                d[
                    ORIGIN_TEMPLATE
    ...
    config = {
        "version": "04c-pca-price-v8",
        "year": args.year,
        "parameter_model_mode": (
            parameter_bundle[
                "model_mode"
            ]
        ),
        ...
        "uniform_price_quantity_grid": True,
        "q_shares_used_for": (
            "segment/breakpoint prediction and evaluation"
        ),
