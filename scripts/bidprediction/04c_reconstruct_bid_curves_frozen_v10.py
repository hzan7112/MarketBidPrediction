...
def v10_predict_price_standardized(
    price_info,
    X,
    Xs,
):
    predictor = price_info[
        "predictor"
    ]
    ...
    if kind == "Ridge":
        ...
        raise KeyError(
            f"Unknown price predictor kind: {kind}"
        )
...
def v9_predict_curve(
    ...
    z_price = v10_predict_price_standardized(
        bundle[
            "price_model"
        ],
        X,
...
def main():
    ...
    if (
        ...
        and parameter_bundle.get(
            "model_mode"
        )
        != "direct_21_price_hybrid_q_specialist"
    ):
        raise RuntimeError(
            "This 04c expects the v9/v10 direct-price 04b bundle. "
            "Run 04b_train_template_parameter_models_frozen_v10.py first."
        )
    ...
    print("=" * 80)
    print(
        "Final bid-curve prediction - direct 21-point price curve + hybrid extrapolation"
    )
    print("=" * 80)
    print(
        f"Year:                  {args.year}"
    ...
    predictor_meta = parameter_bundle[
        "price_model"
    ].get(
        "predictor",
        {},
    ...
    if predictor_meta:
        print(
            "Price predictor kind: "
            f"{predictor_meta.get('kind', 'unknown')}"
        )
    print(
    ...
    lines = [
        ...
        (
            "  price predictor kind = "
            f"{parameter_bundle['price_model'].get('predictor', {}).get('kind', 'legacy')}"
        ),
        "",
    ...
    config = {
        "version": "04c-direct-price-hybrid-v10",
        "year": args.year,
